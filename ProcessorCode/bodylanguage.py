...
class BL:
    def __init__(keypoints, touchThresh=100):
        ...
        kpt_names = ['nose', 'neck',
                 ...
                 'r_eye', 'l_eye',
                 'r_ear', 'l_ear']
        for i, name in kpt_names:
            self.kpt_dict[name] = keypoints[i] 
        self.result = pipeline()

    def scaler(): #calculates a scaling factor based upon a very rough idea of how far away someone is
        avgHeight = 170 #centimeter
    ...
    def neckTouch():
        leftHandDist = distance(self.kpt_dict['l_wri'], self.kpt_dict['neck'])
        rightHandDist = distance(self.kpt_dict['r_wri'], self.kpt_dict['neck'])
    ...
    def headTouch():
        leftHandDist = distance(self.kpt_dict['l_wri'], self.kpt_dict['head'])
        rightHandDist = distance(self.kpt_dict['r_wri'], self.kpt_dict['head'])
        dist = min([leftHandDist, rightHandDist]) * self.scaler
        if dist < self.touchThresh:
            return True
        else:
    ...
    def pipeline(): #pipe together body language movement decoders
        neck = self.neckTouch()
        head = self.headTouch()
        result = {'neck' : neck, 'head' : head}
        return result
