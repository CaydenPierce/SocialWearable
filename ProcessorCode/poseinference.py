...
def headTouch(right, left, neck, rightShoulder, leftShoulder, topOfHead, cvmat, xScale, yScale):
	...
	headLength = abs(topOfHead[1] - neck[1])
	...
	startRange2 = topOfHead[1] - (headLength * yThreshold)
	endRange2 = topOfHead[1] + (headLength * yThreshold)
	
	cv2.rectangle(cvmat, (int(startRange* xScale) , int(endRange*yScale)), (int(startRange2*xScale), int(endRange2*yScale)), (0,255,0), 2)
...
def checkForActions(cvmat, kps, scale_x, scale_y):
	...
	if ((rightHand or leftHand) and neck and leftShoulder and rightShoulder and waist):
		print("enough points to detect neck touch")
	...
	if ((rightHand or leftHand) and neck and leftShoulder and rightShoulder and topOfHead):
		...
		triggered, x, y = neckTouch(rightHand, leftHand, neck, rightShoulder, leftShoulder, topOfHead, cvmat, 4 * scale_x, 4 * scale_y)
		if triggered:
...
def main(args, model, in_res_h, in_res_w, image, frame):
	...
	return actionDetected, actionName
